...
func TestGetFuturePrice(t *testing.T) {
	// Switch to test mode
	...
	tests := []struct {
		name           string
		symbol         string
		mockResponse   *models.ResponseBinanceFuture
		mockStatusCode int
		expectedStatus int
		expectedError  string
	}{
		{
			name:   "successful future price fetch",
			symbol: "BTCUSDT",
			mockResponse: &models.ResponseBinanceFuture{
				Symbol:               "BTCUSDT",
				MarkPrice:            "50000.00",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expectedError != "" {
				...
				assert.Equal(t, tt.mockResponse.MarkPrice, response["price"])
